...
import { Edit, Lock, Search, Trash2, X, Percent, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function Taxes() {
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('taxes.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create tax.')); },
            });
        } else {
            router.put(route('taxes.update', currentItem.id), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update tax.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('taxes.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete tax')}: ${Object.values(errors).join(', ')}`); },
        });
    };

    const handleToggleStatus = (item: any) => {
        router.put(route('taxes.toggle-status', item.id), {}, {
            onSuccess: (page) => {
    ...
    return (
        <PageTemplate title={t('Taxes')} description={t('Manage tax rates applied to your products and orders.')} url="/taxes" breadcrumbs={breadcrumbs} noPadding>
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                ...
                                <Input id="name" type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder={t('e.g. VAT, GST, Sales Tax')} className={formErrors.name ? 'border-red-500' : ''} disabled={!canCreate && !canEdit} required />
                                {formErrors.name && <p className="text-sm text-red-500">{formErrors.name}</p>}
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="type">{t('Type')}</Label>
                                <Select value={formData.type} onValueChange={(value) => setFormData({ ...formData, type: value })} disabled={!canCreate && !canEdit}>
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="rate" type="number" step="0.0001" min="0" max="999.9999" value={formData.rate} onChange={(e) => setFormData({ ...formData, rate: parseFloat(e.target.value) || 0 })} placeholder={t('e.g. 10, 7.5, 20')} className={formErrors.rate ? 'border-red-500' : ''} disabled={!canCreate && !canEdit} />
                                {formErrors.rate && <p className="text-sm text-red-500">{formErrors.rate}</p>}
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="description">{t('Description')}</Label>
                                <Textarea id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder={t('Enter tax description...')} rows={3} className={formErrors.description ? 'border-red-500' : ''} disabled={!canCreate && !canEdit} />
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && <Button type="submit" className="flex-1">{formMode === 'create' ? t('Add Tax') : t('Update Tax')}</Button>}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search taxes...')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch(e)} className="pl-10" />
                                </div>
                            ...
                            <>
                                        ...
                                        <thead className="bg-gray-50 dark:bg-gray-700">
                                            <tr className="bg-[#F0F0F1] hover:bg-[#F0F0F1] dark:border-gray-900 dark:bg-gray-900 border-t">
                                                <th className="cursor-pointer px-4 py-3 text-left text-xs font-medium tracking-wider text-gray-500 select-none dark:text-gray-300" onClick={() => handleSort('name')}>
                                                    <div className="flex items-center gap-1">{t('Tax')}{pageFilters.sort_field === 'name' ? (pageFilters.sort_direction === 'asc' ? ' ↑' : ' ↓') : <span className="opacity-40">↕</span>}</div>
                                                </th>
                                                <th className="cursor-pointer px-3 py-3 text-left text-xs font-medium tracking-wider text-gray-500 select-none dark:text-gray-300" onClick={() => handleSort('rate')}>
                                                    <div className="flex items-center gap-1">{t('Rate')}{pageFilters.sort_field === 'rate' ? (pageFilters.sort_direction === 'asc' ? ' ↑' : ' ↓') : <span className="opacity-40">↕</span>}</div>
                                                </th>
                                        ...
                                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                            {taxes.data.map((item: any) => (
                                                <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                    ...
                                                    <td className="px-3 py-4 text-sm text-gray-700 dark:text-gray-300">{item.type === 'percentage' ? t('Percentage') : t('Fixed')}</td>
                                        ...
                                        <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                            <div className="mb-3 flex items-start justify-between">
                                                <div className="flex gap-3">
                                                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><Percent className="h-5 w-5" /></div>
                                                    <div>
                                            ...
                                            <div className="mt-3 grid grid-cols-3 gap-4 border-t border-gray-100 pt-3 dark:border-gray-700">
                                                <div><p className="mb-1 text-xs text-gray-500 dark:text-gray-400">{t('Rate')}</p><span className="text-sm font-medium text-gray-900 dark:text-white">{formatRate(item)}</span></div>
                                                <div><p className="mb-1 text-xs text-gray-500 dark:text-gray-400">{t('Type')}</p><span className="text-sm font-medium text-gray-900 dark:text-white">{item.type === 'percentage' ? t('Percentage') : t('Fixed')}</span></div>
                                ...
                                {taxes?.total > (taxes?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination from={taxes?.from || 0} to={taxes?.to || 0} total={taxes?.total || 0} links={taxes?.links} entityName={t('taxes')} hidePerPage={true} onPageChange={(url) => router.get(url, {}, { preserveState: true, preserveScroll: true })} />
                                    </div>
                                )}
                            </>
                        ) : (
                            <div className="p-12 text-center">
                                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700"><Percent className="h-8 w-8 text-gray-400" /></div>
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No taxes found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">{hasActiveFilters() ? t('No taxes match your search criteria. Try adjusting your filters.') : t('Create taxes to apply to your products.')}</p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first tax.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
            </div>
            <CrudDeleteModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={handleDeleteConfirm} itemName={currentItem?.name || ''} entityName={t('tax')} />
        </PageTemplate>
    );
}
