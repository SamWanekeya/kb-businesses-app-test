...
import { Edit, Lock, Search, Trash2, X, Bell, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AnnouncementCategories() {
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('announcement-categories.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('announcement-categories.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create announcement category.')); },
            });
        } else {
            router.put(route('announcement-categories.update', currentItem.id), formData, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update announcement category.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('announcement-categories.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete announcement category')}: ${Object.values(errors).join(', ')}`); },
        });
    };
    ...
    const handleToggleStatus = (item: any) => {
        router.put(route('announcement-categories.toggle-status', item.id), {}, {
            ...
            onError: (errors) => toast.error(`${t('Failed to update announcement category')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-announcement-categories');
    const canEdit = hasPermission(permissions, 'edit-announcement-categories');
    const canDelete = hasPermission(permissions, 'delete-announcement-categories');
    const canToggleStatus = hasPermission(permissions, 'toggle-status-announcement-categories');
    ...
    return (
        <PageTemplate title={t('Announcement Categories')} description={t('Manage announcement categories for your announcements.')} breadcrumbs={breadcrumbs} noPadding>
                ...
                <div className="lg:col-span-1">
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{formMode === 'create' ? t('Add New Category') : t('Edit Category')}</h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{formMode === 'create' ? t('Fill in the details to create a new announcement category') : t('Update the announcement category details below')}</p>
                        </div>
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder={t('Enter category description...')} rows={3} className={formErrors.description ? 'border-red-500' : ''} disabled={!canCreate && !canEdit} />
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && <Button type="submit" className="flex-1">{formMode === 'create' ? t('Add Category') : t('Update Category')}</Button>}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search announcement categories...')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch(e)} className="pl-10" />
                                </div>
                        ...
                        {(categories?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Announcement Categories')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage categories for your announcements.')}</p>
                                </div> */}
                                        ...
                                        <thead className="bg-gray-50 dark:bg-gray-700">
                                            <tr className="bg-[#F0F0F1] hover:bg-[#F0F0F1] dark:border-gray-900 dark:bg-gray-900 border-t">
                                                <th className="cursor-pointer px-4 py-3 text-left text-xs font-medium tracking-wider text-gray-500 select-none dark:text-gray-300" onClick={() => handleSort('name')}>
                                                    <div className="flex items-center gap-1">{t('Category')}{pageFilters.sort_field === 'name' ? (pageFilters.sort_direction === 'asc' ? ' ↑' : ' ↓') : <span className="opacity-40">↕</span>}</div>
                                                </th>
                                        ...
                                        <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                            <div className="mb-3 flex items-start justify-between">
                                                <div className="flex gap-3">
                                                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><Bell className="h-5 w-5" /></div>
                                                    <div>
                                ...
                                {categories?.total > (categories?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination from={categories?.from || 0} to={categories?.to || 0} total={categories?.total || 0} links={categories?.links} entityName={t('announcement categories')} hidePerPage={true} onPageChange={(url) => router.get(url, {}, { preserveState: true, preserveScroll: true })} />
                                    </div>
                                )}
                            </>
                        ) : (
                            <div className="p-12 text-center">
                                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700"><Bell className="h-8 w-8 text-gray-400" /></div>
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No announcement categories found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">{hasActiveFilters() ? t('No announcement categories match your search criteria. Try adjusting your filters.') : t('Create announcement categories to organize your announcements.')}</p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first announcement category.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
            </div>
            <CrudDeleteModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={handleDeleteConfirm} itemName={currentItem?.name || ''} entityName={t('announcement category')} />
        </PageTemplate>
    );
}
