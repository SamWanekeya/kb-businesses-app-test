...
import { Bell, ChevronDown, ChevronUp, Edit, Lock, Search, Trash2, X } from 'lucide-react';
import { useCallback, useEffect, useRef, useState } from 'react';
import { useTranslation } from 'react-i18next';

export default function AnnouncementCategories() {
    const { t: translate } = useTranslation();
    const { auth, categories, filters: pageFilters = {} } = usePage().props;
    ...
    const applyFilters = (status = selectedStatus, search = searchTerm) => {
        router.get(
            route('announcement-categories.index'),
            {
                page: 1,
        ...
        if (formMode === 'create') {
            router.post(route('announcement-categories.store'), formData, {
                ...
                onError: (errors) => {
                    ...
                    toast.error(translate('Failed to create announcement category.'));
                },
            });
        } else {
            router.put(route('announcement-categories.update', currentItem.id), formData, {
                ...
                onError: (errors) => {
                    setFormErrors(errors);
                    toast.error(translate('Failed to update announcement category.'));
                },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('announcement-categories.destroy', currentItem.id), {
            ...
            onError: (errors) => {
                setIsDeleteModalOpen(false);
                toast.error(`${translate('Failed to delete announcement category')}: ${Object.values(errors).join(', ')}`);
            },
        });
    };
    ...
    const handleToggleStatus = (item: any) => {
        router.put(
            route('announcement-categories.toggle-status', item.id),
            {},
            {
                onSuccess: (page) => {
                    if (page.props.flash.success) {
                ...
                onError: (errors) => toast.error(`${translate('Failed to update announcement category')}: ${Object.values(errors).join(', ')}`),
            },
        );
    };
    ...
    const canCreate = useHasPermission('create-announcement-categories');
    const canEdit = useHasPermission('edit-announcement-categories');
    const canDelete = useHasPermission('delete-announcement-categories');
    const canToggleStatus = useHasPermission('toggle-status-announcement-categories');
    ...
    return (
        <PageTemplate
            title={translate('Announcement Categories')}
            description={translate('Manage announcement categories for your announcements.')}
        ...
        >
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            ...
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create'
                                    ? translate('Fill in the details to create a new announcement category')
                                    : translate('Update the announcement category details below')}
                            </p>
                        </div>
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder={translate('e.g. Product Updates, Organization News, Events')}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? translate('Add Category') : translate('Update Category')}
                                    </Button>
                                )}
                                {formMode === 'edit' && (
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input
                                        type="text"
                                        placeholder={translate('Search announcement categories...')}
                                        value={searchTerm}
                        ...
                        {(categories?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{translate('Announcement Categories')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{translate('Manage categories for your announcements.')}</p>
                                </div> */}
                                ...
                                {categories?.total > (categories?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            ...
                                            entityName={translate('announcement categories')}
                                            hidePerPage={true}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{translate('No announcement categories found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters()
                                        ? translate('No announcement categories match your search criteria. Try adjusting your filters.')
                                        : translate('Create announcement categories to organize your announcements.')}
                                </p>
                                {!hasActiveFilters() && canCreate && (
                                    <p className="text-sm text-gray-500 dark:text-gray-400">
                                        {translate('Use the form on the left to add your first announcement category.')}
                                    </p>
                                )}
                            </div>
                        )}
            ...
            <CrudDeleteModal
                ...
                entityName={translate('announcement category')}
            />
        </PageTemplate>
    );
}
