      ...
      <div className="mx-auto space-y-6">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-bold leading-none">{formatCurrency(purchaseOrder.total_amount)}</h3>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-bold leading-none">{purchaseOrder.products?.length || 0}</h3>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-bold leading-tight">{window.appSettings?.formatDateTime(purchaseOrder.order_date, false) || new Date(purchaseOrder.order_date).toLocaleDateString()}</h3>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-bold leading-tight">{window.appSettings?.formatDateTime(purchaseOrder.expected_delivery_date, false) || new Date(purchaseOrder.expected_delivery_date).toLocaleDateString()}</h3>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {purchaseOrder.account && (
                <div className="p-6 bg-green-50 rounded-xl border border-green-200 hover:shadow-md hover:bg-green-50/80 hover:border-green-300 transition-all cursor-pointer">
                  ...
                  {hasPermission(permissions, 'view-accounts') ? (
                    ...
                    <p className="text-base font-medium text-green-700">{purchaseOrder.account.name}</p>
                  )}
                </div>
              )}
              ...
              {purchaseOrder.sales_order && (
                <div className="p-6 bg-purple-50 rounded-xl border border-purple-200 hover:shadow-md hover:bg-purple-50/80 hover:border-purple-300 transition-all cursor-pointer">
                  ...
                  {hasPermission(permissions, 'view-sales-orders') ? (
                    ...
                    <p className="text-base font-medium text-purple-700">{purchaseOrder.sales_order.name}</p>
                  )}
                </div>
              )}
              ...
              {purchaseOrder.shipping_provider_type && (
                <div className="p-6 bg-orange-50 rounded-xl border border-orange-200 hover:shadow-md hover:bg-orange-50/80 hover:border-orange-300 transition-all cursor-pointer">
                  ...
                  {hasPermission(permissions, 'view-shipping-provider-types') ? (
                    <Link
                    ...
                    <p className="text-base font-medium text-orange-700">{purchaseOrder.shipping_provider_type.name}</p>
                  )}
                </div>
              )}
            </div>
                ...
                <Table>
                  ...
                  <TableBody>
                      ...
                      return (
                        <TableRow key={index} className="border-b hover:bg-gray-50">
                          ...
                          <TableCell className="text-right font-bold text-base py-4 px-4">
                            <span className="text-green-600 font-semibold">{formatCurrency(finalTotal + taxAmount)}</span>
                          </TableCell>
                        </TableRow>
                      );
                    })}
                    <TableRow className="bg-gray-50 border-t-2">
                      ...
                      <TableCell className="text-right font-semibold text-base py-3 px-4">{formatCurrency(purchaseOrder.subtotal)}</TableCell>
                    </TableRow>
                    <TableRow className="bg-gray-50">
                      <TableCell colSpan={4} className="py-3 px-4"></TableCell>
                      <TableCell className="text-right font-semibold text-base py-3 px-4">{t('Discount')}:</TableCell>
                      <TableCell className="text-right font-semibold text-base text-red-600 py-3 px-4">-{formatCurrency(purchaseOrder.discount_amount)}</TableCell>
                    </TableRow>
                    <TableRow className="bg-gray-50">
                      <TableCell colSpan={4} className="py-3 px-4"></TableCell>
                      <TableCell className="text-right font-semibold text-base py-3 px-4">{t('Tax')}:</TableCell>
                      <TableCell className="text-right font-semibold text-base py-3 px-4">{formatCurrency(purchaseOrder.tax_amount)}</TableCell>
                    </TableRow>
                    <TableRow className="bg-green-50 border-t-2">
                      ...
                      <TableCell className="text-right py-4 px-4"><span className="text-green-600 font-bold text-xl">{formatCurrency(purchaseOrder.total_amount)}</span></TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              </div>
        ...
        <Card className="shadow-sm">
          ...
          {showStream && (
            <CardContent className="p-0">
              {/* Add Comment Form */}
              {hasPermission(permissions, 'create-purchase-orders') && <div className="sticky top-0 bg-white z-10 p-6 pb-6 border-b">
                                ...
                                <div>
                                  {editingComment === activity.id ? (
                                    ...
                                    <div className="flex items-start justify-between">
                                      ...
                                      {((activity.user_id === auth?.user?.id ) && hasPermission(permissions, 'edit-purchase-orders') )&& (
                                        <Button
                                          size="sm"
                                          variant="ghost"
