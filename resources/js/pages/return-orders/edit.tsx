...
export default function ReturnOrderEdit() {
    ...
    const buildReturnItems = (prods: any[], salesOrderProds: any[] = []): ReturnItem[] =>
        prods.map((p: any) => {
            ...
            return {
                ...
                discount_type: soProduct?.discount_type ?? p.pivot?.discount_type ?? p.discount_type ?? 'none',
                discount_value: soProduct?.discount_value ?? p.pivot?.discount_value ?? p.discount_value ?? 0,
                tax: prod?.tax ?? null,
    ...
    return (
        <PageTemplate title={t('Edit Return Order')} breadcrumbs={breadcrumbs} url="/return-orders" fullWidth
