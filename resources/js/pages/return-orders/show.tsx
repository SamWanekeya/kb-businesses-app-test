            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-bold leading-none">{formatCurrency(returnOrder.total_amount)}</h3>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-bold leading-none">{getReasonLabel(returnOrder.reason)}</h3>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-bold leading-none">{returnOrder.products?.length || 0}</h3>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-bold leading-tight">{window.appSettings?.formatDateTime(returnOrder.return_date, false) || new Date(returnOrder.return_date).toLocaleDateString()}</h3>
                </div>
          ...
          <CardContent className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {returnOrder.salesOrder && (
                <div className="p-6 bg-blue-50 rounded-xl border border-blue-200 hover:shadow-md hover:bg-blue-50/80 hover:border-blue-300 transition-all cursor-pointer">
                  ...
                  {hasPermission(permissions, 'view-sales-orders') ? (
                    <Link
                      href={route('sales-orders.show', returnOrder.salesOrder.id)}
                      className="text-base font-medium hover:underline" style={{ color: '#2563eb' }}>
                      {returnOrder.salesOrder.order_number} - {returnOrder.salesOrder.name}
                    </Link>
                  ) : (
                    <p className="text-base font-medium text-blue-600">{returnOrder.salesOrder.order_number} - {returnOrder.salesOrder.name}</p>
                  )}
                </div>
              )}
              ...
              {returnOrder.account && (
                <div className="p-6 bg-green-50 rounded-xl border border-green-200 hover:shadow-md hover:bg-green-50/80 hover:border-green-300 transition-all cursor-pointer">
                  ...
                  {hasPermission(permissions, 'view-accounts') ? (
                    ...
                    <p className="text-base font-medium text-green-600">{returnOrder.account.name}</p>
                  )}
                </div>
              )}
              ...
              {returnOrder.contact && (
                <div className="p-6 bg-purple-50 rounded-xl border border-purple-200 hover:shadow-md hover:bg-purple-50/80 hover:border-purple-300 transition-all cursor-pointer">
                  ...
                  {hasPermission(permissions, 'view-contacts') ? (
                    ...
                    <p className="text-base font-medium text-purple-600">{returnOrder.contact.name}</p>
                  )}
                </div>
              )}
            </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    <TableRow className="bg-gray-50 border-t-2">
                      <TableCell colSpan={3} className="py-3 px-4"></TableCell>
                      <TableCell className="text-right font-semibold text-base py-3 px-4">{t('Subtotal')}:</TableCell>
                      <TableCell className="text-right font-semibold text-base py-3 px-4">{formatCurrency(returnOrder.subtotal)}</TableCell>
                    </TableRow>
                    <TableRow className="bg-gray-50">
                      <TableCell colSpan={3} className="py-3 px-4"></TableCell>
                      <TableCell className="text-right font-semibold text-base py-3 px-4">{t('Tax')}:</TableCell>
                      <TableCell className="text-right font-semibold text-base py-3 px-4">{formatCurrency(returnOrder.tax_amount)}</TableCell>
                    </TableRow>
                    <TableRow className="bg-green-50 border-t-2">
                      <TableCell colSpan={3} className="py-4 px-4"></TableCell>
                      <TableCell className="text-right font-bold text-lg py-4 px-4">{t('Grand Total')}:</TableCell>
                      <TableCell className="text-right py-4 px-4"><span className="text-green-600 font-bold text-xl">{formatCurrency(returnOrder.total_amount)}</span></TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              </div>
