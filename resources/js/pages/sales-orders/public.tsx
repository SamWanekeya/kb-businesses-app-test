...
import { Printer, Copy, Check, Calendar, DollarSign, Package, User, Clock, MessageCircle, Truck } from 'lucide-react';
...
export default function PublicSalesOrder({ salesOrder, templateId = 'template1', color = 'ffffff', qrEnabled = false, settings = {}, themeColor = 'blue', customColor = null }: Props) {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 print-container">
                    ...
                    <div className="mx-auto space-y-6 print:hidden">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                            {([
                                { label: t('Total Amount'), value: <span className="font-mono">{formatCurrency(salesOrder.total_amount)}</span>, icon: DollarSign, iconCls: 'text-emerald-600', blobCls: 'bg-emerald-50' },
                                { label: t('Products'), value: salesOrder.products?.length || 0, icon: Package, iconCls: 'text-blue-600', blobCls: 'bg-blue-50' },
                                { label: t('Order Date'), value: formatDate(salesOrder.order_date), icon: Calendar, iconCls: 'text-orange-600', blobCls: 'bg-orange-50' },
                                { label: t('Delivery Date'), value: formatDate(salesOrder.delivery_date), icon: Clock, iconCls: 'text-purple-600', blobCls: 'bg-purple-50' },
                            ...
                            <div className="p-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            {salesOrder.creator ? (
                                                <div className="flex items-center gap-2 mt-2">
                                                    <Avatar className="w-8 h-8 flex-shrink-0">
                                                        ...
                                                        <AvatarFallback><UserInitials name={salesOrder.creator.name} /></AvatarFallback>
                                                    </Avatar>
                                                    <div className="min-w-0">
                                                        <p className="text-sm font-medium text-gray-900 truncate">{salesOrder.creator.name}</p>
                                                        {salesOrder.creator.email && <p className="text-xs text-gray-500 truncate">{salesOrder.creator.email}</p>}
                                                    </div>
                                                </div>
                                            ...
                                            {salesOrder.assigned_user ? (
                                                <div className="flex items-center gap-2 mt-2">
                                                    <Avatar className="w-8 h-8 flex-shrink-0">
                                                        ...
                                                        <AvatarFallback><UserInitials name={salesOrder.assigned_user.name} /></AvatarFallback>
                                                    </Avatar>
                                                    <div className="min-w-0">
                                                        <p className="text-sm font-medium text-gray-900 truncate">{salesOrder.assigned_user.name}</p>
                                                        {salesOrder.assigned_user.email && <p className="text-xs text-gray-500 truncate">{salesOrder.assigned_user.email}</p>}
                                                    </div>
                                                </div>
                                            ) : <p className="text-sm mt-1 text-gray-500">{t('-')}</p>}
                                        </div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div>
                                            <label className="text-sm font-medium text-muted-foreground">{t('Order Date')}</label>
                                        ...
                                        <div>
                                            <label className="text-sm font-medium text-muted-foreground">{t('Delivery Date')}</label>
                                            <div className="flex items-center gap-2 mt-1">
                                                <Calendar className="h-4 w-4 text-gray-400 flex-shrink-0" />
                            ...
                            <div className="bg-white rounded-lg shadow-sm border">
                                ...
                                <div className="p-6">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {salesOrder.account && (
                                            <div className="p-4 rounded-xl border border-gray-200 hover:shadow-md transition-shadow">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="text-sm font-semibold text-gray-900 truncate">{salesOrder.account.name}</p>
                                                        {salesOrder.account.email && <p className="text-xs text-gray-500 truncate">{salesOrder.account.email}</p>}
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                        {salesOrder.quote && (
                                            <div className="p-4 rounded-xl border border-gray-200 hover:shadow-md transition-shadow">
                                                <p className="text-xs font-medium text-gray-500 mb-3">{t('Quote')}</p>
                                                <div className="flex items-center gap-2">
                                                    <div className="min-w-0">
                                                        <p className="text-sm font-semibold text-gray-900 truncate">{salesOrder.quote.name}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                        {salesOrder.shipping_provider_type && (
                                            <div className="p-4 rounded-xl border border-gray-200 hover:shadow-md transition-shadow">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="text-sm font-semibold text-gray-900 truncate">{salesOrder.shipping_provider_type.name}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                    ...
                                    <div>
                                        ...
                                        {salesOrder.billing_contact && (
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-sm font-medium text-foreground truncate">{salesOrder.billing_contact.name}</p>
                                    ...
                                    <div className="sm:border-s sm:ps-6">
                                        ...
                                        {salesOrder.shipping_contact && (
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-sm font-medium text-foreground truncate">{salesOrder.shipping_contact.name}</p>
