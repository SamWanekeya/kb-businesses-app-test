...
import { ChevronDown, ChevronUp, Edit, LayoutGrid, Lock, Search, Trash2, X } from 'lucide-react';
...
export default function Categories() {
        ...
        if (formMode === 'create') {
            router.post(route('categories.store'), formData, {
                ...
                onError: (errors) => {
                    ...
                    toast.error(translate('Failed to create category.'));
                },
            });
        } else {
            router.put(route('categories.update', currentItem.id), formData, {
                ...
                onError: (errors) => {
                    setFormErrors(errors);
                    toast.error(translate('Failed to update category.'));
                },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('categories.destroy', currentItem.id), {
            ...
            onError: (errors) => {
                setIsDeleteModalOpen(false);
                toast.error(`${translate('Failed to delete category')}: ${Object.values(errors).join(', ')}`);
            },
        });
    };
    ...
    const handleToggleStatus = (item: any) => {
        router.put(
            route('categories.toggle-status', item.id),
            {},
            {
                onSuccess: (page) => {
                    if (page.props.flash.success) {
                ...
                onError: (errors) => toast.error(`${translate('Failed to update category')}: ${Object.values(errors).join(', ')}`),
            },
        );
    };
    ...
    const canCreate = useHasPermission('create-categories');
    const canEdit = useHasPermission('edit-categories');
                        ...
                        {(categories?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{translate('Categories')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{translate('Manage categories for your products.')}</p>
                                </div> */}
                                {/* Desktop Table */}
                                        ...
                                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                            {categories.data.map((item: any) => (
                                                <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                    ...
                                                    <td className="px-3 py-4 font-mono text-sm text-gray-700 dark:text-gray-300">
                                                        {item.slug || '-'}
                                                    </td>
                                    ...
                                    {categories.data.map((item: any) => (
                                        ...
                                        >
                                            ...
                                            <div className="mt-3 grid grid-cols-2 gap-4 border-t border-gray-100 pt-3 dark:border-gray-700">
                                                <div>
                                                    <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">{translate('Slug')}</p>
                                                    <span className="font-mono text-sm text-gray-900 dark:text-white">{item.slug || '-'}</span>
                                                </div>
                                                <div>
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{translate('No categories found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters()
                                        ? translate('No categories match your search criteria. Try adjusting your filters.')
                                        : translate('Create categories to organize your products.')}
                                </p>
                                {!hasActiveFilters() && canCreate && (
                                    <p className="text-sm text-gray-500 dark:text-gray-400">
                                        {translate('Use the form on the left to add your first category.')}
                                    </p>
                                )}
                            </div>
                        )}
