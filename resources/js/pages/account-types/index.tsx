...
import { Edit, Lock, Search, Trash2, X, Building2, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AccountTypes() {
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('account-types.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create account type.')); },
            });
        } else {
            router.put(route('account-types.update', currentItem.id), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update account type.')); },
            });
        }
    };
    ...
    const handleToggleStatus = (item: any) => {
        router.put(route('account-types.toggle-status', item.id), {}, {
            onSuccess: (page) => {
    ...
    return (
        <PageTemplate title={t('Account Types')} description={t('Manage account type categories for your accounts.')} url="/account-types" breadcrumbs={breadcrumbs} noPadding>
                        ...
                        {(accountTypes?.data || []).length > 0 ? (
                            ...
                            <div className="p-12 text-center">
                                ...
                                {!hasActiveFilters() && canCreate && (
                                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first account type.')}</p>
                                )}
                            </div>
                        )}
                    </div>
