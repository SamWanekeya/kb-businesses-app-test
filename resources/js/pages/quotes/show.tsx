...
import { ArrowLeft, Calendar, Package, Building2, FileText, Trash2, Send, Edit, MessageCircle, Clock, Truck, User, Eye, Check } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-6">
        ...
        {/* Products */}
        <Card className="shadow-sm overflow-hidden" id="products-section">
          <CardHeader className="border-b py-3.5 px-5">
          ...
          <CardContent className="p-0">
            {quote.products && quote.products.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-sm font-medium text-muted-foreground">{t('No products added to this quote')}</p>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="p-0">
            <div className="max-h-[150px] overflow-y-auto">
              <div className="px-5 py-4">
                <p className="text-sm text-foreground whitespace-pre-line leading-relaxed">{quote.description || t('-')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {hasPermission(permissions, 'view-stream') && (
          <Card className="shadow-sm">
            ...
            <CardContent className="p-0">
              {hasPermission(permissions, 'create-quotes') && (
                <div className="px-5 pt-4 pb-4 border-b">
                  <form onSubmit={(e) => {
                    e.preventDefault();
                    if (newComment.trim()) {
                      router.post(route('quotes.comments.store', quote.id), { comment: newComment }, { preserveScroll: true, onSuccess: () => setNewComment('') });
                    }
                  }}>
                            ...
                            <div className={`flex items-center justify-between gap-2 px-4 py-2.5 ${isEditing ? 'bg-emerald-50/60' : 'bg-muted/30'} border-b`}>
                              ...
                              {!isEditing && (
                                <div className="flex items-center gap-1 flex-shrink-0">
                                  {activity.activity_type === 'comment' && activity.user_id === auth?.user?.id && hasPermission(permissions, 'edit-quotes') && (
                                    <TooltipProvider delayDuration={200}>
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                  ...
                                  <div className="space-y-3">
                                    ...
                                    <div className="flex items-center justify-end gap-2">
                                      ...
                                      <Button size="sm" className="bg-emerald-500 hover:bg-emerald-600 text-white" onClick={() => {
                                        router.put(route('quotes.comments.update-activity', { quote: quote.id, activity: activity.id }), { comment: editCommentText }, { preserveScroll: true });
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-2xl font-bold font-mono text-foreground">{formatCurrency(quote.total_amount)}</p>
              </div>
        ...
        {(quote.contact || quote.account || quote.billing_contact || quote.shipping_contact) && (
          <Card className="shadow-sm">
            ...
            <CardContent className="p-0">
              ...
              {quote.contact && quote.account && <div className="border-t mx-0" />}
              {quote.account && (
                <div className="px-4 pt-3 pb-3">
                  <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1">
                    <Building2 className="h-3.5 w-3.5 text-muted-foreground shrink-0" />{t('Account')}
                  </p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 min-w-0">
                      <UserInitials name={quote.account.name} />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{quote.account.name}</p>
                        {quote.account.email && <p className="text-xs text-muted-foreground truncate">{quote.account.email}</p>}
                      </div>
                    </div>
              ...
              {(quote.contact || quote.account) && quote.billing_contact && <div className="border-t mx-0" />}
              {quote.billing_contact && (
                <div className="px-4 pt-3 pb-3">
                  <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1">
                    <User className="h-3.5 w-3.5 text-muted-foreground shrink-0" />{t('Billing Contact')}
                  </p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 min-w-0">
                      <UserInitials name={quote.billing_contact.name} />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{quote.billing_contact.name}</p>
                    ...
                    {hasPermission(permissions, 'view-contacts') && (
                      <TooltipProvider delayDuration={200}>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Link href={route('contacts.show', quote.billing_contact.id)} className="ms-3 flex-shrink-0">
                              <Eye className="h-4 w-4 text-muted-foreground" />
                            </Link>
                          </TooltipTrigger>
                          <TooltipContent side="top"><p>{t('View')}</p></TooltipContent>
              ...
              {quote.billing_contact && quote.shipping_contact && <div className="border-t mx-0" />}
              {quote.shipping_contact && (
                <div className="px-4 pt-3 pb-3">
                  <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1">
                    <User className="h-3.5 w-3.5 text-muted-foreground shrink-0" />{t('Shipping Contact')}
                  </p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 min-w-0">
                      <UserInitials name={quote.shipping_contact.name} />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{quote.shipping_contact.name}</p>
                    ...
                    {hasPermission(permissions, 'view-contacts') && (
                      <TooltipProvider delayDuration={200}>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Link href={route('contacts.show', quote.shipping_contact.id)} className="ms-3 flex-shrink-0">
                              <Eye className="h-4 w-4 text-muted-foreground" />
                            </Link>
                          </TooltipTrigger>
                          <TooltipContent side="top"><p>{t('View')}</p></TooltipContent>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="p-5 space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">{t('Quote Number')}</p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">{t('Quote Date')}</p>
                <p className="text-sm font-medium text-foreground">{formatDate(quote.quote_date || quote.created_at)}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <Calendar className="h-4 w-4 text-muted-foreground mt-0.5 shrink-0" />
              <div>
                <p className="text-xs text-muted-foreground">{t('Valid Until')}</p>
                <p className="text-sm font-medium text-foreground">{formatDate(quote.valid_until)}</p>
              </div>
            </div>
            {quote.assigned_user && (
              <div className="border-t pt-3">
                ...
                <div className="flex items-center gap-2">
                  <Avatar className="w-8 h-8 flex-shrink-0">
                    ...
                    <AvatarFallback className="bg-primary/15 text-primary text-xs font-bold">{getInitials(quote.assigned_user.name || '')}</AvatarFallback>
                  </Avatar>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{quote.assigned_user.name}</p>
                    {quote.assigned_user.email && <p className="text-xs text-muted-foreground truncate">{quote.assigned_user.email}</p>}
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="border-t pt-3">
                ...
                <div className="flex items-center gap-2">
                  <Avatar className="w-8 h-8 flex-shrink-0">
                    ...
                    <AvatarFallback className="bg-primary/15 text-primary text-xs font-bold">{getInitials(quote.creator.name || '')}</AvatarFallback>
                  </Avatar>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{quote.creator.name}</p>
                    {quote.creator.email && <p className="text-xs text-muted-foreground truncate">{quote.creator.email}</p>}
                  </div>
                </div>
              </div>
            )}
        ...
        {(quote.shipping_provider_type || quote.opportunity) && (
          <Card className="shadow-sm">
            ...
            <CardContent className="p-5 space-y-2">
              {quote.opportunity && hasPermission(permissions, 'view-opportunities') && (
                <Link href={route('opportunities.show', quote.opportunity.id)} className="flex items-center justify-between p-2.5 rounded-lg border hover:bg-muted/40 transition-colors">
                  <div className="min-w-0">
                    <p className="text-xs text-muted-foreground">{t('Opportunity')}</p>
                    <p className="text-sm font-medium text-foreground truncate">{quote.opportunity.name}</p>
                  </div>
                  <TooltipProvider delayDuration={200}>
                    <Tooltip>
                      <TooltipTrigger asChild>
              ...
              {quote.shipping_provider_type && hasPermission(permissions, 'view-shipping-provider-types') && (
                <Link href={route('shipping-provider-types.show', quote.shipping_provider_type.id)} className="flex items-center justify-between p-2.5 rounded-lg border hover:bg-muted/40 transition-colors">
                  <div className="min-w-0">
                    <p className="text-xs text-muted-foreground">{t('Shipping Provider')}</p>
                    <p className="text-sm font-medium text-foreground truncate">{quote.shipping_provider_type.name}</p>
                  </div>
                  <TooltipProvider delayDuration={200}>
                    <Tooltip>
                      <TooltipTrigger asChild>
