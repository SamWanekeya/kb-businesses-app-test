...
export default function QuoteEdit() {
    ...
    const { data, setData, setError, clearErrors, put, processing, errors } = useForm({
        ...
        opportunity_id: String(quote.opportunity_id || ''),
        account_id: String(quote.account_id || ''),
        billing_contact_id: String(quote.billing_contact_id || ''),
        shipping_contact_id: String(quote.shipping_contact_id || ''),
        shipping_provider_type_id: String(quote.shipping_provider_type_id || ''),
        status: quote.status || 'draft',
        valid_until: quote.valid_until ? quote.valid_until.substring(0, 10) : '',
        assigned_to: String(quote.assigned_to || ''),
        ...
        products: (quote.products?.length ? quote.products : [{ id: '', pivot: null }]).map((p: any, idx: number) => ({
            id: crypto.randomUUID(),
            product_id: p.id ? String(p.id) : '',
            quantity: String(p.pivot?.quantity || 1),
            unit_price: String(p.pivot?.unit_price || p.price || 0),
            discount_type: p.pivot?.discount_type || 'none',
            discount_value: String(p.pivot?.discount_value || '0'),
        })) as ProductRow[],
    });
    ...
    const removeProductRow = (rowId: string) => {
        ...
        setData(
            'products',
            data.products.filter((r) => r.id !== rowId),
        );
    };

    const updateProductRow = (rowId: string, field: string, value: string) => {
        const idx = data.products.findIndex((r) => r.id === rowId);
        ...
        if (idx >= 0) clearErrors(`products.${idx}.${field}` as any);
                            ...
                            <table className="block w-full text-sm xl:table">
                                ...
                                <tbody className="block space-y-4 divide-y divide-gray-200 xl:table-row-group xl:space-y-0 xl:divide-y-0">
                                    {data.products.map((row, i) => {
                                        ...
                                        const usedIds = data.products.filter((r) => r.id !== row.id && r.product_id).map((r) => r.product_id);
                                        const lineProductOptions = productOptions.filter(
                                            (p: any) => !usedIds.includes(String(p.id)) || String(p.id) === row.product_id,
                                        );
                                        return (
                                            <tr
                                                key={row.id}
                        ...
                        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <Textarea value={data.shipping_address} onChange={(e) => set('shipping_address', e.target.value)} rows={2} />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <Label className="text-sm font-medium">{translate('City')}</Label>
                                        <Input value={data.shipping_city} onChange={(e) => set('shipping_city', e.target.value)} />
                                    </div>
                                    <div className="space-y-1">
                                        <Label className="text-sm font-medium">{translate('State')}</Label>
                                        <Input value={data.shipping_state} onChange={(e) => set('shipping_state', e.target.value)} />
                                    </div>
                                    <div className="space-y-1">
                                        <Label className="text-sm font-medium">{translate('Country')}</Label>
                                        <Input value={data.shipping_country} onChange={(e) => set('shipping_country', e.target.value)} />
                                    </div>
                                    <div className="space-y-1">
                                        <Label className="text-sm font-medium">{translate('Postal Code')}</Label>
                                        <Input value={data.shipping_postal_code} onChange={(e) => set('shipping_postal_code', e.target.value)} />
                                    </div>
                                </div>
                            </div>
                        </div>
