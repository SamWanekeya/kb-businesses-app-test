...
import { CheckSquare, ChevronDown, ChevronUp, Edit, Lock, Search, Trash2, X } from 'lucide-react';
...
export default function TaskStatuses() {
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        setFormErrors({});
        if (formMode === 'create') {
            const toastId = toast.loading(translate('Creating task status...'));
    ...
    const handleDeleteConfirm = () => {
        const toastId = toast.loading(translate('Deleting task status...'));
    ...
    const handleToggleStatus = (item: any) => {
        ...
        router.put(
            route('task-statuses.toggle-status', item.id),
            {},
            {
                onSuccess: () => {
                    toast.dismiss(toastId);
    ...
    const canCreate = useHasPermission('create-task-statuses');
    const canEdit = useHasPermission('edit-task-statuses');
