...
export default function Brands() {
        ...
        if (formMode === 'create') {
            router.post(route('brands.store'), formData, {
                ...
                onError: (errors) => {
                    ...
                    toast.error(t('Failed to create brand.'));
                },
            });
        } else {
            router.put(route('brands.update', currentItem.id), formData, {
                ...
                onError: (errors) => {
                    setFormErrors(errors);
                    toast.error(t('Failed to update brand.'));
                },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('brands.destroy', currentItem.id), {
            ...
            onError: (errors) => {
                setIsDeleteModalOpen(false);
                toast.error(`${t('Failed to delete brand')}: ${Object.values(errors).join(', ')}`);
            },
        });
    };
    ...
    return (
        <PageTemplate
            title={t('Brands')}
            description={t('Manage brands associated with your products.')}
            url="/brands"
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Brand') : t('Edit Brand')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new brand') : t('Update the brand details below')}
                            </p>
                        </div>
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder={t('e.g. Nike, Apple, Samsung')}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Brand') : t('Update Brand')}
                                    </Button>
                                )}
                                {formMode === 'edit' && (
                        ...
                        {(brands?.data || []).length > 0 ? (
                                    ...
                                    {brands.data.map((item: any) => (
                                        ...
                                        >
                                            ...
                                            <div className="mt-3 grid grid-cols-2 gap-4 border-t border-gray-100 pt-3 dark:border-gray-700">
                                                <div>
                                                    <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">{t('Website')}</p>
                                                    {item.website ? (
                                                        <a
                                                            href={item.website}
                                                            target="_blank"
                            ...
                            <div className="p-12 text-center">
                                ...
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters()
                                        ? t('No brands match your search criteria. Try adjusting your filters.')
                                        : t('Create brands to associate with your products.')}
                                </p>
                                {!hasActiveFilters() && canCreate && (
                                    <p className="text-sm text-gray-500 dark:text-gray-400">
                                        {t('Use the form on the left to add your first brand.')}
                                    </p>
                                )}
                            </div>
                        )}
