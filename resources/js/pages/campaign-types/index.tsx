...
import { ChevronDown, ChevronUp, Edit, Lock, Megaphone, Search, Trash2, X } from 'lucide-react';
...
export default function CampaignTypes() {
        ...
        if (formMode === 'create') {
            router.post(route('campaign-types.store'), formData, {
                ...
                onError: (errors) => {
                    ...
                    toast.error(t('Failed to create campaign type.'));
                },
            });
        } else {
            router.put(route('campaign-types.update', currentItem.id), formData, {
                ...
                onError: (errors) => {
                    setFormErrors(errors);
                    toast.error(t('Failed to update campaign type.'));
                },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('campaign-types.destroy', currentItem.id), {
            ...
            onError: (errors) => {
                setIsDeleteModalOpen(false);
                toast.error(`${t('Failed to delete campaign type')}: ${Object.values(errors).join(', ')}`);
            },
        });
    };
    ...
    const breadcrumbs = [{ title: t('Dashboard'), href: route('dashboard') }, { title: t('Campaign Management') }, { title: t('Campaign Types') }];

    return (
        <PageTemplate
            title={t('Campaign Types')}
            description={t('Manage campaign type categories for your campaigns.')}
            url="/campaign-types"
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Campaign Type') : t('Edit Campaign Type')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create'
                                    ? t('Fill in the details to create a new campaign type')
                                    : t('Update the campaign type details below')}
                            </p>
                        </div>
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder={t('e.g. Email Campaign, Social Media, Webinar')}
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    ...
                                    placeholder={t('Enter campaign type description...')}
                                    rows={3}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Campaign Type') : t('Update Campaign Type')}
                                    </Button>
                                )}
                                {formMode === 'edit' && (
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input
                                        type="text"
                                        placeholder={t('Search campaign types...')}
                                        value={searchTerm}
                        ...
                        {(campaignTypes?.data || []).length > 0 ? (
                            <>
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead className="bg-gray-50 dark:bg-gray-700">
                                            <tr className="border-t bg-[#F0F0F1] hover:bg-[#F0F0F1] dark:border-gray-900 dark:bg-gray-900">
                                                ...
                                                <th className="stext-right px-4 py-3 pr-[50px] text-xs font-medium tracking-wider text-gray-500 dark:text-gray-300">
                                                    {t('Actions')}
                                                </th>
                                            </tr>
                                        </thead>
                                ...
                                <div className="space-y-4 p-4 lg:hidden">
                                    {campaignTypes.data.map((item: any) => (
                                        ...
                                        >
                                            <div className="mb-3 flex w-full flex-col items-start justify-between gap-2 min-[341px]:flex-row min-[341px]:gap-4">
                                                ...
                                                <div className="flex w-full justify-end gap-1 border-t pt-2 min-[341px]:w-auto min-[341px]:border-t-0 min-[341px]:pt-0">
                                                    {canEdit && (
                                                        <TooltipProvider>
                                                            <Tooltip>
                                                                <TooltipTrigger asChild>
                            ...
                            <div className="p-12 text-center">
                                ...
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters()
                                        ? t('No campaign types match your search criteria. Try adjusting your filters.')
                                        : t('Create campaign types to categorize your campaigns.')}
                                </p>
                                {!hasActiveFilters() && canCreate && (
                                    <p className="text-sm text-gray-500 dark:text-gray-400">
                                        {t('Use the form on the left to add your first campaign type.')}
                                    </p>
                                )}
                            </div>
                        )}
