...
import { Edit, Lock, Search, Trash2, X, Megaphone, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function CampaignTypes() {
    ...
    }, [checkOverflow, campaignTypes?.data]);
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('campaign-types.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('campaign-types.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create campaign type.')); },
            });
        } else {
            router.put(route('campaign-types.update', currentItem.id), formData, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update campaign type.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('campaign-types.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete campaign type')}: ${Object.values(errors).join(', ')}`); },
        });
    };

    const handleToggleStatus = (item: any) => {
        router.put(route('campaign-types.toggle-status', item.id), {}, {
            onSuccess: (page) => {
            ...
            onError: (errors) => toast.error(`${t('Failed to update campaign type')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-campaign-types');
    const canEdit = hasPermission(permissions, 'edit-campaign-types');
    const canDelete = hasPermission(permissions, 'delete-campaign-types');
    const canToggleStatus = hasPermission(permissions, 'toggle-status-campaign-types');
                ...
                <div className="lg:col-span-1">
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{formMode === 'create' ? t('Add New Campaign Type') : t('Edit Campaign Type')}</h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{formMode === 'create' ? t('Fill in the details to create a new campaign type') : t('Update the campaign type details below')}</p>
                        </div>
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                <Label htmlFor="name" required>{t('Name')}</Label>
                                <Input id="name" type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder={t('e.g. Email Campaign, Social Media, Webinar')} className={formErrors.name ? 'border-red-500' : ''} disabled={!canCreate && !canEdit} required />
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder={t('Enter campaign type description...')} rows={3} className={formErrors.description ? 'border-red-500' : ''} disabled={!canCreate && !canEdit} />
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && <Button type="submit" className="flex-1">{formMode === 'create' ? t('Add Campaign Type') : t('Update Campaign Type')}</Button>}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search campaign types...')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch(e)} className="pl-10" />
                                </div>
                    ...
                    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        {(campaignTypes?.data || []).length > 0 ? (
                                ...
                                <div className="hidden lg:block overflow-x-auto">
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                            {campaignTypes.data.map((item: any) => (
                            ...
                            <div className="p-12 text-center">
                                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700"><Megaphone className="h-8 w-8 text-gray-400" /></div>
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No campaign types found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">{hasActiveFilters() ? t('No campaign types match your search criteria. Try adjusting your filters.') : t('Create campaign types to categorize your campaigns.')}</p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first campaign type.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
            </div>
            <CrudDeleteModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={handleDeleteConfirm} itemName={currentItem?.name || ''} entityName={t('campaign type')} />
        </PageTemplate>
    );
}
