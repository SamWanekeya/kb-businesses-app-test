...
class QuoteObserver
{
    ...
    public function created(Quote $quote): void
    {
        ...
        QuoteActivity::create([
            'quote_id' => $quote->id,
            'user_id' => $creator->id ?? $quote->created_by,
            'activity_type' => 'created',
            'title' => ($creator->name ?? 'System') . ' created this quote',
        ...
        if (!$selfAssigned && $quote->assignedUser) {
            ...
            QuoteActivity::create([
                ...
                'user_id' => $creator->id ?? $quote->created_by,
                'activity_type' => 'assigned',
                'title' => ($creator->name ?? 'System') . ' self-assigned this quote',
                'description' => '',
                'new_values' => ['assigned_to' => $quote->assigned_to],
                'created_by' => $quote->created_by,
            ]);
    ...
    private function getUpdateDescription($field, $oldValue, $newValue, $quote)
    {
        switch ($field) {
            ...
            case 'opportunity_id':
                $oldOpportunity = $oldValue ? Opportunity::find($oldValue)?->name : 'None';
                $newOpportunity = $newValue ? Opportunity::find($newValue)?->name : 'None';
