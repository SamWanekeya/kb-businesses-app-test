...
class PurchaseOrderObserver
{
    public function created(PurchaseOrder $purchaseOrder): void
    {
        ...
        PurchaseOrderActivity::create([
            ...
            'user_id' => $creator->id ?? $purchaseOrder->created_by,
            'activity_type' => 'created',
            'title' => ($creator->name ?? 'System') . ' created this purchase order',
        ...
        if (!$selfAssigned && $purchaseOrder->assignedUser) {
            ...
            PurchaseOrderActivity::create([
                ...
                'user_id' => $creator->id ?? $purchaseOrder->created_by,
                'activity_type' => 'assigned',
                'title' => ($creator->name ?? 'System') . ' self-assigned this purchase order',
                'description' => '',
                'new_values' => ['assigned_to' => $purchaseOrder->assigned_to],
                'created_by' => $purchaseOrder->created_by,
            ]);
    ...
    private function getUpdateDescription($field, $oldValue, $newValue, $purchaseOrder)
    {
        switch ($field) {
            ...
            case 'sales_order_id':
                $oldSalesOrder = $oldValue ? SalesOrder::find($oldValue)?->order_number : 'None';
                $newSalesOrder = $newValue ? SalesOrder::find($newValue)?->order_number : 'None';
